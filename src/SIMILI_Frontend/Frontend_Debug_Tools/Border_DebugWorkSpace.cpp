...
Border_DebugWorkSpace::Border_DebugWorkSpace()
	: vk_context_(nullptr)
...
bool Border_DebugWorkSpace::initialize(VKContext* vkContext, VkRenderPass renderPass)
{
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to create graphics resources" << std::endl;
		return false;
	}

	initialized_ = true;
	std::cout << "[Border_DebugWorkSpace] Successfully initialized" << std::endl;
	return true;
}

void Border_DebugWorkSpace::shutdown()
{
	destroyGraphicsResources();
	initialized_ = false;
}

void Border_DebugWorkSpace::setVulkanPipelines(VulkanPipeline* pipelines)
{
	vulkan_pipelines_ = pipelines;
}

void Border_DebugWorkSpace::draw(VkCommandBuffer commandBuffer, int drawableWidth, int drawableHeight, const SIMILI::Frontend::WorkSpace* workspace)
{
	if (!initialized_)
	{
		return;
	...
	{
		std::cerr << "[Border_DebugWorkSpace] draw() - Pipeline is null!" << std::endl;
		return;
	}
	...
	int refWidth = drawableWidth;
	int refHeight = drawableHeight;
	...
	int left = workspace->getX();
	int top  = workspace->getY();
	int right = workspace->getX() + workspace->getWidth();
	int bottom = workspace->getY() + workspace->getHeight();
...
bool Border_DebugWorkSpace::createGraphicsResources()
{
	if (!vk_context_ || !vulkan_pipelines_ || !vulkan_pipelines_->isInitialized())
	{
		std::cerr << "[Border_DebugWorkSpace] Missing VulkanPipeline or VKContext" << std::endl;
		return false;
	}

	if (vk_render_pass_ == VK_NULL_HANDLE)
	{
		std::cerr << "[Border_DebugWorkSpace] RenderPass is VK_NULL_HANDLE" << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to compile vertex shader: " << GLSLCompiler::getLastError() << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to compile fragment shader: " << GLSLCompiler::getLastError() << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to create vertex buffer" << std::endl;
		return false;
	}
	...
	if (vkAllocateMemory(device, &allocInfo, nullptr, &vk_vertex_buffer_memory_) != VK_SUCCESS)
	{
		...
		vk_vertex_buffer_ = VK_NULL_HANDLE;
		std::cerr << "[Border_DebugWorkSpace] Failed to allocate vertex buffer memory" << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to create descriptor set layout" << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to create descriptor pool" << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to allocate descriptor set" << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to create dummy texture image" << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to allocate dummy texture memory" << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to create dummy texture view" << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to create sampler" << std::endl;
		return false;
	}
	...
	config.name = "border_debug_workspace";
	...
	if (!shared_pipeline_ || shared_pipeline_->pipeline == VK_NULL_HANDLE)
	{
		std::cerr << "[Border_DebugWorkSpace] Failed to create pipeline" << std::endl;
		return false;
	}

	std::cout << "[Border_DebugWorkSpace] createGraphicsResources() completed" << std::endl;
	return true;
}

void Border_DebugWorkSpace::destroyGraphicsResources()
{
	if (!vk_context_)
	{
		return;
...
uint32_t Border_DebugWorkSpace::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
