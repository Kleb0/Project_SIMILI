...
void HierarchyListDrawing::drawChildSlots(ThreeDObject* parent,  bool& clickedOnItem)
{
	...
	for (ThreeDObject* child : parent->getChildren())
	{
		...
		static std::set<ThreeDObject*> drawnChildren;
		if (drawnChildren.find(child) == drawnChildren.end()) 
		{
			std::cout << "[HierarchyListDrawing] Child drawn: " << child->getName() << std::endl;
			drawnChildren.insert(child);
		}
